...
def gestionar_objeto(request):
    obras = listar_obra(request)
    obra_id = request.POST['obra']
    list_obj = Objeto.objects.all().select_related("obra").filter(obra__usuarios = request.user.id, obra_id = obra_id).order_by('codigo')
...
def gestionar_plano(request):
    ...
    form.fields['actividad'].queryset = Actividad.objects.filter(orden_trab_id=obra.orden_trab_id).order_by('codigo_act')
...
def adicionar_objeto(request):
        ...
        if form.is_valid():
            ...
            if Objeto.objects.filter(obra=obj.obra, codigo=obj.codigo).count():
                context = {'list_obj': list_obj, 'form': form, 'object': obj,
                           'errores': 'La obra: ( ' + obra.nombre + ' ) ya tiene un objeto con el código: ( '  + obj.codigo + ' ). Por Favor escoga otro código ', 'obras': obras }
                context = context_add_perm(request, context, 'prenomina15', 'objeto')
                return render(request, 'Gestionar_Objeto.html', context)
            else:
                try:
    ...
    context = {'list_obj': list_obj, 'form': form, 'obras': obras, 'success': '!!! El objeto: (' + obra.nombre + '/' + obj.codigo + '/' + obj.nombre +' ) ha sido creado satisfactoriamente !!!'}
    context = context_add_perm(request, context, 'prenomina15', 'objeto')
    return render(request, 'Gestionar_Objeto.html', context)
...
def registrar_revision(request):
    ...
    if request.method == 'POST':
        if formrev.is_valid():
            ...
            if aprobado == 'SI':
                plano.rev_pago = revision.no_rev
                plano.fecha_pago = revision.fecha_revision
...
def editar_objeto(request, pk):
    ...
    cal = Objeto.objects.all().select_related("obra").filter(obra__usuarios=request.user.id, obra_id=obra.id).order_by('codigo')
    ...
    if form.is_valid():
        if form.cleaned_data['obra'] != obra or codigo != form.cleaned_data['codigo']:
            ...
            if nombre != form.cleaned_data['nombre']:
                obj.save()
                cal = Objeto.objects.all().select_related("obra").filter(obra__usuarios=request.user.id, obra_id=obra.id).order_by('codigo')
                context = {'list_obj': cal, 'form': form, 'obras': obras, 'success': '!!! El objeto: (' + obra.nombre + '/' + obj.codigo + '/' + obj.nombre +' ) ha sido modificado satisfactoriamente !!!'}

            else:
                cal = Objeto.objects.all().select_related("obra").filter(obra__usuarios=request.user.id, obra_id=obra.id).order_by('codigo')
                context = {'list_obj': cal, 'form': form, 'obras': obras, 'warning': '!!! El objeto: (' + obra.nombre + '/' + obj.codigo + '/' + obj.nombre +' ) no ha sufrido modificaciones !!!'}
...
def editar_plano(request, pk):
    ...
    cal = Plano.objects.all().select_related("actividad").select_related("objeto").select_related(
        "formato").select_related("obra").select_related("especialidad").select_related("trabajador").filter(obra__usuarios=request.user.id, obra_id=plano.obra_id)
    plano_filter = PlanoFilter(request.POST, queryset=cal)
    ...
    if form.is_valid():
        ...
        if (numero != for_numero) or (objeto != for_objeto) or (act != for_act) or (obra != for_obra):
            if Plano.objects.filter(num=for_numero, objeto=for_objeto, actividad=for_act, obra=for_obra).count():
                ...
                plano.cant = form.cleaned_data['cant']
...
def editar_revision(request, pk):
    ...
    if request.method == 'POST':
        if formrev.is_valid():
            ...
            if request.POST["aprobado"]:
                aprobado = request.POST["aprobado"]
                if aprobado == 'SI':
                    plano.rev_pago = rev.no_rev
            ...
            context = {'list_rev': cal, 'formrev': formrev, 'obras': obras, 'edit': rev.id, 'nombre': nombre,
                       'objeto': objeto, 'num': num, 'pk': rev.plano_id}
            context = context_add_perm(request, context, 'prenomina15', 'revision')
            return render(request, 'Listado_Revisiones.html', context)
    else:
        cal = Revision.objects.filter(plano_id=rev.plano_id)
...
def list_acta(request):
    ...
    if request.POST:
        ...
        if list_act:
            ...
            obra = list_act[0].plano.obra.orden_trab.descripcion_ot
            ...
            context = {'list_act': list_act, 'list_planos': list_planos, 'act': act, 'obras': obras, 'obra': obra,
                       'fecha': fecha,
                       'obra_id': obra_id, 'elaborado_por': elaborado_por, 'recibido_por': recibido_por,
                       'actas_activas': actas_activas}
            context = context_add_perm(request, context, 'prenomina15', 'acta')
...
def listar_obra(request):
    obras = Obra.objects.all().filter(usuarios=request.user.id)
    return obras


@permission_required('prenomina15.read_acta', 'home_principal')
def buscar_acta(request, obra_id, recibido_por=None):
    obras = listar_obra(request)
    list_act = Revision.objects.all().filter(entregado__contains='Acta', plano__obra_id=obra_id).distinct(
        'entregado').order_by('entregado')
    acta = request.POST['acta']
    list_planos = Revision.objects.filter(entregado=acta, plano__obra_id=obra_id).select_related(
        'plano').select_related(
        'plano__trabajador').select_related('plano__actividad').select_related('plano__objeto')
    act = list_planos[0].entregado
    obra = list_planos[0].plano.obra.orden_trab.descripcion_ot
    ...
    elaborado_por = request.user
    context = {'list_act': list_act, 'list_planos': list_planos, 'act': act, 'obras': obras, 'obra': obra,
               'fecha': fecha, 'obra_id': obra_id, 'elaborado_por': elaborado_por, 'recibido_por': recibido_por,
               'actas_activas': actas_activas}
    context = context_add_perm(request, context, 'prenomina15', 'acta')
    return render(request, 'Buscar_Acta.html', context)
...
def exportar_acta_entrega(request, fecha, rp):
    list_rev = Revision.objects.filter(fecha_estado=fecha).select_related("plano").select_related(
        "plano__trabajador").select_related("plano__objeto").select_related(
        "plano__especialidad").select_related("plano__actividad").select_related("plano__obra").select_related(
...
def request_report(fecha_inic, fecha_fin, obra, request):
    sql = "Select ges_trab_trabajador.primer_nombre, ges_trab_trabajador.segundo_nombre,prenomina15_plano.fecha_vpc," \
          "ges_trab_trabajador.apellidos, adm_cargo.nombre as nom_cargo, adm_escalasalarial.grupo, prenomina15_salariomax.sal, " \
          "prenomina15_plano.formato_id, prenomina15_plano.porciento, prenomina15_plano.tarifa, prenomina15_plano.cant, " \
          "entrada_datos_actividad.codigo_act, prenomina15_plano.horas_creadas, prenomina15_plano.valor, " \
          "prenomina15_plano.valor_total, prenomina15_plano.valor_retenido, prenomina15_plano.nombre," \
          " prenomina15_plano.num, prenomina15_objeto.codigo, prenomina15_plano.last_rev, prenomina15_plano.fecha_pago, " \
          "prenomina15_especialidad.nombre as nombre_esp, prenomina15_plano.trabajador_id, prenomina15_plano.id," \
          "prenomina15_objeto.nombre as nombre_obj, prenomina15_plano.corte, prenomina15_plano.horas_creadas_real," \
          "prenomina15_plano.valor_real, prenomina15_plano.valor_retenido_real, prenomina15_plano.valor_total_real," \
          "prenomina15_plano.vpc, prenomina15_especialidad.siglas" \
          " FROM public.ges_trab_trabajador, public.prenomina15_plano, public.adm_cargo, " \
          " public.adm_escalasalarial, public.prenomina15_salariomax, public.prenomina15_obra," \
          " public.prenomina15_formato, public.entrada_datos_actividad, public.prenomina15_objeto, public.prenomina15_especialidad " \
          "WHERE ges_trab_trabajador.id = prenomina15_plano.trabajador_id and " \
          "adm_cargo.id = ges_trab_trabajador.cargo_id and prenomina15_salariomax.grupo_esc_id = adm_escalasalarial.id" \
          " and adm_escalasalarial.id = ges_trab_trabajador.escala_salarial_id and " \
          " prenomina15_plano.obra_id =" "'" + obra + "'" " and  prenomina15_obra.tipo = prenomina15_salariomax.tipo and " \
                                                      "prenomina15_formato.id = prenomina15_plano.formato_id and entrada_datos_actividad.id = " \
                                                      "prenomina15_plano.actividad_id and prenomina15_objeto.id = prenomina15_plano.objeto_id and " \
                                                      "prenomina15_plano.especialidad_id = prenomina15_especialidad.id and " \
                                                      "(prenomina15_plano.fecha_pago BETWEEN " + "'" + fecha_inic + "'" + "::DATE AND " + "'" + fecha_fin + "'" + "::DATE " \
                                                                                                                                                                  "or prenomina15_plano.fecha_vpc BETWEEN " + "'" + fecha_inic + "'" + "::DATE AND " + "'" + fecha_fin + "'" + "::DATE)  " \
                                                                                                                                                                                                                                                                               "group by prenomina15_plano.vpc, prenomina15_especialidad.nombre, ges_trab_trabajador.primer_nombre, " \
                                                                                                                                                                                                                                                                               "ges_trab_trabajador.segundo_nombre, ges_trab_trabajador.apellidos, adm_cargo.nombre, " \
                                                                                                                                                                                                                                                                               "adm_escalasalarial.grupo, prenomina15_salariomax.sal, prenomina15_formato.id, " \
                                                                                                                                                                                                                                                                               "prenomina15_plano.porciento, prenomina15_plano.tarifa, entrada_datos_actividad.codigo_act, " \
                                                                                                                                                                                                                                                                               "prenomina15_plano.horas_creadas, prenomina15_plano.valor, prenomina15_plano.valor_total," \
                                                                                                                                                                                                                                                                               " prenomina15_plano.valor_retenido, prenomina15_plano.nombre, prenomina15_plano.num, prenomina15_especialidad.siglas, " \
                                                                                                                                                                                                                                                                               "prenomina15_objeto.codigo, prenomina15_plano.last_rev, prenomina15_plano.fecha_pago, prenomina15_plano.fecha_vpc, " \
                                                                                                                                                                                                                                                                               "prenomina15_plano.trabajador_id, prenomina15_plano.id, prenomina15_objeto.nombre, prenomina15_plano.corte order by " \
                                                                                                                                                                                                                                                                               "prenomina15_especialidad.nombre, ges_trab_trabajador.primer_nombre, ges_trab_trabajador.segundo_nombre, " \
                                                                                                                                                                                                                                                                               "ges_trab_trabajador.apellidos, adm_escalasalarial.grupo, prenomina15_plano.horas_creadas_real," \
                                                                                                                                                                                                                                                                               " prenomina15_plano.valor_real, prenomina15_plano.valor_retenido_real, prenomina15_plano.valor_total_real," \
    ...
    for element in result:
        ...
        planos.append(
            Plan(nombre=element.nombre, codigo=element.num, objeto=element.codigo, etapa=element.codigo_act,
                 ...
                 tarifa=element.tarifa, sigla=element.siglas))
...
def request_report_pren(fecha_inic, fecha_fin, obra, request):
    sql = "Select ges_trab_trabajador.primer_nombre, ges_trab_trabajador.segundo_nombre,prenomina15_plano.fecha_vpc," \
          "ges_trab_trabajador.apellidos, adm_escalasalarial.grupo, prenomina15_salariomax.sal, " \
          "prenomina15_plano.formato_id, prenomina15_plano.porciento, prenomina15_plano.tarifa, prenomina15_plano.cant, " \
          "entrada_datos_actividad.codigo_act, prenomina15_plano.horas_creadas, prenomina15_plano.valor, " \
          "prenomina15_plano.valor_total, prenomina15_plano.valor_retenido, prenomina15_plano.nombre," \
          " prenomina15_plano.num, prenomina15_objeto.codigo, prenomina15_plano.last_rev, prenomina15_plano.fecha_pago, " \
          ...
          "prenomina15_objeto.nombre as nombre_obj, prenomina15_plano.corte, prenomina15_plano.horas_creadas_real," \
          " prenomina15_plano.valor_real, prenomina15_plano.valor_retenido_real, prenomina15_plano.valor_total_real," \
          "prenomina15_plano.vpc, ges_trab_trabajador.departamento_id, adm_departamento.nombre as nombre_dpto,  adm_departamento.codigo," \
          "ges_trab_trabajador.codigo_interno, ges_trab_trabajador.ci, ges_trab_trabajador.categoria, ges_trab_trabajador.cargo_id," \
          "ges_trab_trabajador.salario_escala, ges_trab_trabajador.cies, ges_trab_trabajador.incre_res, adm_cargo.nombre as nombre_cargo, " \
          "ges_trab_trabajador.sal_cat_cient, ges_trab_trabajador.antiguedad, ges_trab_trabajador.salario_total, prenomina15_especialidad.siglas" \
          " FROM public.ges_trab_trabajador, public.prenomina15_plano, public.adm_cargo," \
          " public.adm_escalasalarial, public.prenomina15_salariomax, public.prenomina15_obra," \
          " public.prenomina15_formato, public.entrada_datos_actividad, public.prenomina15_objeto, " \
          " public.prenomina15_especialidad, public.adm_departamento WHERE ges_trab_trabajador.id = prenomina15_plano.trabajador_id and " \
          " prenomina15_salariomax.grupo_esc_id = adm_escalasalarial.id and ges_trab_trabajador.cargo_id = adm_cargo.id" \
          " and adm_escalasalarial.id = ges_trab_trabajador.escala_salarial_id and adm_departamento.id = ges_trab_trabajador.departamento_id and" \
          " prenomina15_plano.obra_id =" "'" + obra + "'" " and  prenomina15_obra.tipo = prenomina15_salariomax.tipo and " \
                                                      "prenomina15_formato.id = prenomina15_plano.formato_id and entrada_datos_actividad.id = " \
                                                      "prenomina15_plano.actividad_id and prenomina15_objeto.id = prenomina15_plano.objeto_id and " \
                                                      "prenomina15_plano.especialidad_id = prenomina15_especialidad.id and " \
                                                      "(prenomina15_plano.fecha_pago BETWEEN " + "'" + fecha_inic + "'" + "::DATE AND " + "'" + fecha_fin + "'" + "::DATE " \
                                                                                                                                                                  "or prenomina15_plano.fecha_vpc BETWEEN " + "'" + fecha_inic + "'" + "::DATE AND " + "'" + fecha_fin + "'" + "::DATE)  " \
                                                                                                                                                                                                                                                                               "group by prenomina15_especialidad.siglas, prenomina15_plano.vpc,adm_departamento.codigo, ges_trab_trabajador.departamento_id, nombre_dpto, prenomina15_especialidad.nombre, ges_trab_trabajador.primer_nombre, " \
                                                                                                                                                                                                                                                                               "ges_trab_trabajador.segundo_nombre, ges_trab_trabajador.apellidos, ges_trab_trabajador.categoria, ges_trab_trabajador.cargo_id, " \
                                                                                                                                                                                                                                                                               " adm_escalasalarial.grupo, prenomina15_salariomax.sal, prenomina15_formato.id, ges_trab_trabajador.ci, adm_cargo.nombre, " \
                                                                                                                                                                                                                                                                               "prenomina15_plano.porciento, prenomina15_plano.tarifa, entrada_datos_actividad.codigo_act, ges_trab_trabajador.cies," \
                                                                                                                                                                                                                                                                               "prenomina15_plano.horas_creadas, prenomina15_plano.valor, prenomina15_plano.valor_total, ges_trab_trabajador.salario_escala," \
                                                                                                                                                                                                                                                                               " prenomina15_plano.valor_retenido, prenomina15_plano.nombre, prenomina15_plano.num, ges_trab_trabajador.codigo_interno, " \
                                                                                                                                                                                                                                                                               "prenomina15_objeto.codigo, prenomina15_plano.last_rev, prenomina15_plano.fecha_pago, prenomina15_plano.fecha_vpc," \
                                                                                                                                                                                                                                                                               " ges_trab_trabajador.cies, ges_trab_trabajador.incre_res, ges_trab_trabajador.sal_cat_cient, ges_trab_trabajador.antiguedad," \
                                                                                                                                                                                                                                                                               "prenomina15_plano.trabajador_id, prenomina15_plano.id, prenomina15_objeto.nombre, prenomina15_plano.corte, ges_trab_trabajador.salario_total order by adm_escalasalarial.grupo," \
                                                                                                                                                                                                                                                                               "prenomina15_especialidad.nombre, ges_trab_trabajador.primer_nombre, ges_trab_trabajador.segundo_nombre, " \
                                                                                                                                                                                                                                                                               "ges_trab_trabajador.apellidos, adm_escalasalarial.grupo, prenomina15_plano.horas_creadas_real," \
                                                                                                                                                                                                                                                                               " prenomina15_plano.valor_real, prenomina15_plano.valor_retenido_real, prenomina15_plano.valor_total_real;"
    ...
    for element in result:
        ...
        if not flag:
            areas.append(Area(nombre=element.nombre_dpto, codigo=element.codigo, personas=[], cant=0,
                              ...
                              ret=0, ret_ant=0, impacto=0, sal_tot_dev=0))
        ...
        if not flag:
            ...
            personas.append(
                Trab(no=element.codigo_interno, trab_id=element.trabajador_id,
                     ...
                     sal_dev_total=0.00, cargo=element.nombre_cargo))
    ...
    for area in areas:
        for per in area.personas:
            ...
            per.impacto = (per.total_valor - per.total_dev - per.total_retenido) + per.retenido_ant
    ...
    return {'areas': areas, 'fecha_inic': fecha_inic, 'fecha_fin': fecha_fin, 'obras': obras,
            ...
            'impacto': impacto, 'sal_total_dev': sal_total_dev}
...
def request_report_anexo(fecha_inic, fecha_fin, obra, horas, request):
    sql = "Select ges_trab_trabajador.primer_nombre, ges_trab_trabajador.segundo_nombre,prenomina15_plano.fecha_vpc," \
          ...
          " prenomina15_plano.obra_id =" "'" + obra + "'" " and  prenomina15_obra.tipo = prenomina15_salariomax.tipo and " \
                                                      ...
                                                      "(prenomina15_plano.fecha_pago BETWEEN " + "'" + fecha_inic + "'" + "::DATE AND " + "'" + fecha_fin + "'" + "::DATE " \
                                                                                                                                                                  "or prenomina15_plano.fecha_vpc BETWEEN " + "'" + fecha_inic + "'" + "::DATE AND " + "'" + fecha_fin + "'" + "::DATE)  " \
                                                                                                                                                                                                                                                                               "group by prenomina15_plano.vpc,adm_departamento.codigo, ges_trab_trabajador.departamento_id, nombre_dpto, prenomina15_especialidad.nombre, ges_trab_trabajador.primer_nombre, " \
                                                                                                                                                                                                                                                                               "ges_trab_trabajador.segundo_nombre, ges_trab_trabajador.apellidos, ges_trab_trabajador.categoria, ges_trab_trabajador.cargo_id, " \
                                                                                                                                                                                                                                                                               "adm_escalasalarial.grupo, prenomina15_salariomax.sal, prenomina15_formato.id, ges_trab_trabajador.ci, adm_cargo.nombre, " \
                                                                                                                                                                                                                                                                               "prenomina15_plano.porciento, prenomina15_plano.tarifa, entrada_datos_actividad.codigo_act, ges_trab_trabajador.cies, " \
                                                                                                                                                                                                                                                                               "prenomina15_plano.horas_creadas, prenomina15_plano.valor, prenomina15_plano.valor_total, ges_trab_trabajador.salario_escala," \
                                                                                                                                                                                                                                                                               ...
                                                                                                                                                                                                                                                                               "prenomina15_objeto.codigo, prenomina15_plano.last_rev, prenomina15_plano.fecha_pago, prenomina15_plano.fecha_vpc," \
                                                                                                                                                                                                                                                                               " ges_trab_trabajador.cies, ges_trab_trabajador.incre_res, ges_trab_trabajador.sal_cat_cient, ges_trab_trabajador.antiguedad," \
                                                                                                                                                                                                                                                                               "prenomina15_plano.trabajador_id, prenomina15_plano.id, prenomina15_objeto.nombre, prenomina15_plano.corte, ges_trab_trabajador.salario_total order by " \
                                                                                                                                                                                                                                                                               "prenomina15_especialidad.nombre, ges_trab_trabajador.primer_nombre, ges_trab_trabajador.segundo_nombre, " \
                                                                                                                                                                                                                                                                               "ges_trab_trabajador.apellidos, adm_escalasalarial.grupo, prenomina15_plano.horas_creadas_real," \
                                                                                                                                                                                                                                                                               " prenomina15_plano.valor_real, prenomina15_plano.valor_retenido_real, prenomina15_plano.valor_total_real, prenomina15_especialidad.siglas;"
    ...
    for area in areas:
        for per in area.personas:
            ...
            per.dif_30_15 = (per.total_valor - per.total_retenido - per.total_dev) + per.retenido_ant
